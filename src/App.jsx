      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .login-visual {
          ...
          padding: 40px;
          background: linear-gradient(135deg, #29392f 0%, #4a6b5d 55%, #5f8575 130%);
          color: #ffffff;
        }
        .login-visual__rings {
          position: absolute;
...
function ProcedureStepEditor({ step, onChange, onRemove, index }) {
  return (
    <div className="proc-step">
      <div className="proc-step__header">
        <span className="proc-step__number">{index + 1}</span>
        <input
          className="proc-step__title"
          placeholder="Nome da etapa"
          value={step.title}
          onChange={(e) => onChange({ ...step, title: e.target.value })}
        />
        <button type="button" className="proc-step__remove" onClick={onRemove} title="Remover etapa">
          <X size={13} />
        </button>
      </div>
      <textarea
        className="proc-step__docs"
        placeholder="Documentos necessários (um por linha)"
        value={step.documents}
        onChange={(e) => onChange({ ...step, documents: e.target.value })}
      />
    </div>
  );
}

function ProcedureForm({ initial, onSave, onClose, accessToken }) {
  const [title, setTitle] = useState(initial?.title || "");
  const [description, setDescription] = useState(initial?.description || "");
  const [steps, setSteps] = useState(
    initial?.steps?.length ? initial.steps.map((s) => ({ ...s, documents: (s.documents || []).join("\n") })) : [{ title: "", documents: "" }]
  );
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState(null);

  function updateStep(i, next) {
  ...
  function addStep() {
    setSteps((prev) => [...prev, { title: "", documents: "" }]);
  }
  ...
  async function submit(e) {
    ...
    try {
      ...
      if (initial?.id) {
        const [row] = await supaFetch(`procedures?id=eq.${initial.id}`, { method: "PATCH", body: JSON.stringify(body) }, accessToken);
        onSave(row);
      } else {
        const [row] = await supaFetch("procedures", { method: "POST", body: JSON.stringify([body]) }, accessToken);
        onSave(row);
      }
    } catch (e) {
      setError(e.message);
  ...
  return (
    <div className="new-client proc-form">
      <form onSubmit={submit} className="new-client__form">
        <input placeholder="Título do procedimento" required value={title} onChange={(e) => setTitle(e.target.value)} />
        <textarea
          className="proc-form__description"
        ...
        <button type="button" className="proc-add-step" onClick={addStep}>
          <Plus size={13} /> Adicionar etapa
        </button>

        <div className="new-client__actions">
          <button type="button" onClick={onClose} className="new-client__cancel">Cancelar</button>
          <button type="submit" disabled={saving}>{saving ? "Salvando…" : "Salvar procedimento"}</button>
        </div>
        {error && <p className="login-error">Não foi possível salvar: {error}</p>}
      </form>
    </div>
  ...
  return (
    <div className="proc-detail">
      ...
      {procedure.steps.map((step, i) => (
        <div className="proc-detail__step" key={i}>
          <div className="proc-detail__step-title">
            <span className="proc-detail__step-number">{i + 1}</span>
            {step.title}
          </div>
          {step.documents?.length > 0 && (
            <ul className="proc-detail__docs">
...
function ProceduresView({
  ...
  editingProcedure,
  setEditingProcedure,
  confirmingDeleteProcedure,
  setConfirmingDeleteProcedure,
  deleting,
  onProcedureSaved,
  deleteProcedure,
  accessToken,
          ...
          <div className="client-detail-bar">
            <div className="client-detail-bar__row">
              ...
              <div className="client-detail-bar__actions">
                <button onClick={() => setEditingProcedure(true)}>Editar</button>
                <button className="client-detail-bar__delete" onClick={() => setConfirmingDeleteProcedure(true)}>
                  Excluir
                </button>
              </div>
            </div>
            ...
            {confirmingDeleteProcedure && (
              <div className="delete-confirm">
                <p>
                  Tem certeza que quer excluir <strong>{selectedProcedure.title}</strong>? Essa ação não pode ser
                  desfeita.
                </p>
                <div className="delete-confirm__actions">
                  <button className="new-client__cancel" onClick={() => setConfirmingDeleteProcedure(false)}>
                    Cancelar
                  </button>
                  <button
...
export default function App() {
  ...
  const [confirmingDeleteProcedure, setConfirmingDeleteProcedure] = useState(false);
  ...
  async function loadProcedures() {
    try {
      const data = await supaFetch("procedures?select=*&order=created_at.asc", {}, session.accessToken);
      setProcedures(data);
      if (data.length && !selectedProcedureId) setSelectedProcedureId(data[0].id);
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  async function deleteProcedure(id) {
    setDeleting(true);
    try {
      await supaFetch(`procedures?id=eq.${id}`, { method: "DELETE" }, session.accessToken);
      setProcedures((prev) => {
        const next = prev.filter((p) => p.id !== id);
        setSelectedProcedureId(next.length ? next[0].id : null);
        return next;
      });
      setConfirmingDeleteProcedure(false);
    } catch (e) {
  ...
  return (
    <div className="app">
      ...
      <div className="shell">
          ...
          <div className="switcher">
            {isAdmin && (
              <>
                ...
                <button
                  className={view === "procedimentos" ? "active" : ""}
                  onClick={() => {
                    setView("procedimentos");
                    if (!proceduresLoaded) loadProcedures();
                  }}
                >
                  <BookOpen size={13} /> Procedimentos
                </button>
              </>
            )}
        ...
        {isAdmin && view === "procedimentos" && (
          <ProceduresView
            procedures={procedures}
            selectedProcedure={selectedProcedure}
            selectedProcedureId={selectedProcedureId}
            setSelectedProcedureId={setSelectedProcedureId}
            showNewProcedure={showNewProcedure}
            setShowNewProcedure={setShowNewProcedure}
            editingProcedure={editingProcedure}
            setEditingProcedure={setEditingProcedure}
            confirmingDeleteProcedure={confirmingDeleteProcedure}
            setConfirmingDeleteProcedure={setConfirmingDeleteProcedure}
            deleting={deleting}
